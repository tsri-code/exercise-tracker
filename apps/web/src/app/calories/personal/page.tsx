...
export default function PersonalInfoPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (json?.data) {
        ...
        if (json.data.lastTargetKcal) setCustom(json.data.lastTargetKcal);
      }
    })();
  }, []);
  ...
  async function save() {
    ...
    // Normalize nullables to match API schema
    const payload = {
      ...profile,
      customTargetKcal: custom ?? null,
    ...
    if (!resp.ok) {
      console.error("[web] Save failed", resp.status);
    }
    console.log("[web] Saved profile, refetching");
    const res = await fetch(`${API_BASE_URL}/api/nutrition/profile`, {
      cache: "no-store",
    });
    const json = await res.json();
  ...
  function computeTarget() {
    ...
    const weightKg =
      profile.weightUnit === "kg"
        ? profile.weightValue
        : profile.weightValue * LB_TO_KG;
    const heightCm =
      profile.heightUnit === "cm"
        ? profile.heightValue
        : profile.heightValue * IN_TO_CM;
    ...
    if (
      ...
      !Number.isNaN(profile.bodyFatPercent)
    ) {
    ...
    const weeklyDeltaLb =
      profile.goal === "recomp"
        ? 0
        : profile.goal === "gain"
        ? Math.abs(profile.rateLbsPerWeek)
        : -Math.abs(profile.rateLbsPerWeek);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
          <div className="flex gap-2">
            <input
              type="number"
              className="w-full rounded bg-white/5 ring-1 ring-white/10 px-2 py-1"
              placeholder="Weight"
              value={profile.weightValue}
              onChange={(e) =>
                setProfile((p) => ({
                  ...p,
                  weightValue: toNumber(e.target.value, p.weightValue),
                }))
              }
            />
            <select
              ...
              onChange={(e) =>
                setProfile((p) => ({ ...p, weightUnit: e.target.value as any }))
              }
            >
          ...
          <div className="flex gap-2">
            <input
              ...
              onChange={(e) =>
                setProfile((p) => ({
                  ...p,
                  heightValue: toNumber(e.target.value, p.heightValue),
                }))
              }
            />
            <select
              ...
              onChange={(e) =>
                setProfile((p) => ({ ...p, heightUnit: e.target.value as any }))
              }
            >
          ...
          <input
            ...
            onChange={(e) =>
              setProfile((p) => ({
                ...
                age: Math.max(
                  10,
                  Math.min(120, Math.round(toNumber(e.target.value, p.age)))
                ),
              }))
            }
          />
          ...
          <select
            ...
            value={profile.goal}
            onChange={(e) =>
              setProfile((p) => ({ ...p, goal: e.target.value as any }))
            }
          >
          ...
          <div className="flex gap-2">
            <input
              ...
              max={2}
              className="w-full rounded bg-white/5 ring-1 ring-white/10 px-2 py-1"
              placeholder="Rate (lbs/week)"
              value={profile.rateLbsPerWeek}
              onChange={(e) =>
                setProfile((p) => ({
                  ...p,
                  rateLbsPerWeek: Math.max(
                    -2,
                    Math.min(2, toNumber(e.target.value, p.rateLbsPerWeek))
                  ),
                }))
              }
            />
                ...
                bodyFatPercent:
                  e.target.value !== ""
                    ? Math.max(
                        ...
                        Math.min(
                          75,
                          toNumber(e.target.value, p.bodyFatPercent ?? 0)
                        )
                      )
                    : undefined,
              }))
          ...
          <input
            type="text"
            className="rounded bg-white/5 ring-1 ring-white/10 px-2 py-1"
            placeholder="Dietary preference (e.g., keto, vegan)"
            value={profile.dietaryPreference || ""}
            onChange={(e) =>
              setProfile((p) => ({
                ...p,
                dietaryPreference: e.target.value || undefined,
              }))
            }
          />
                  ...
                  e.target.value !== ""
                    ? Math.max(
                        ...
                        Math.min(
                          10,
                          Math.round(
                            toNumber(e.target.value, p.mealsPerDay ?? 1)
                          )
                        )
                      )
                    : undefined,
          ...
          <input
            ...
            max={3}
            className="rounded bg-white/5 ring-1 ring-white/10 px-2 py-1"
            placeholder="Protein g per kg (optional)"
                ...
                proteinPerKg:
                  e.target.value !== ""
                    ? Math.max(
                        ...
                        Math.min(
                          3,
                          toNumber(e.target.value, p.proteinPerKg ?? 0)
                        )
                      )
                    : undefined,
              }))
        ...
        <div className="mt-3">
          ...
          <button
            ...
            className="px-3 py-1 rounded-full text-xs font-semibold text-black bg-white/20 hover:bg-white/30"
          >
            Calculate target
          </button>
        </div>
      ...
      <div className="text-xs text-white/60">
        Healthy goal limits enforced: up to ±2 lbs/week. You can also set a
        custom rate within that range.
      </div>
      {target !== null && (
        <div className="text-sm text-white">
          Target calories (computed):{" "}
          <span className="font-semibold">{target} kcal/day</span>
        </div>
      )}
